...
public class Database extends SQLiteOpenHelper {
    ...
    public ArrayList getOrderData(String username) {
        ...
        str[0] = username;
        Cursor c = db.rawQuery("select * from username where username=? ", str);
        if (c.moveToFirst()) {
            do {
    ...
    public int checkAppointmentExists(String username, String fullname, String address, String contact, String date, String time) {
        ...
        str[5] = time;
        SQLiteDatabase db = getReadableDatabase();
        Cursor c = db.rawQuery("select * from username where username=? and fullname=? and address=? and contactno=? and date=? and time=?", str);
        if (c.moveToFirst()) {
            result = 1;
        }
        db.close();
